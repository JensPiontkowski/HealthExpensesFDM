...
Traditionally, point estimates for the expected claims are obtained through simple robust methods and the loading is added by expert judgement. Here, stochastic models can provide a justification for setting this loading. In practice, variations of the Rusam method are used. The basic underlying idea is that all costs in a year increase by the same factor. To be precise let $y_t(x)$ be the average annual cost of a person of age $x$ in the year $t =  \ldots , T-1, T$. Fix a middle reference age  $x_0$ which has a large number of insured. It is assumed that
$$
...
```{r plot_claims_exposure, fig.cap = 'Graphical reprensentation of the available data',  fig.show = 'hold', out.width='50%'}
oldpar <- par(no.readonly = TRUE)
par(mar=c(5,4,4,2) + 0.1 - 1) #default mar=c(5,4,4,2) + 0.1
plot(Kx, main = paste0("inpatient claims per male (", min(years), "-", max(years), ")"), 
     ylab = "", #colorchoice = "gray", 
...
5. Forecast as follows: Assume we start at time $t=T$. 
...
```{r plot_Kx2_fdm, fig.cap = 'Funtional data model after outlier removal', fig.show = 'hold', out.width='100%'}
oldpar <- par(no.readonly = TRUE)
par(mar = c(5,4,4,2) + 0.1 - 1) #default mar=c(5,4,4,2) + 0.1
plot(Kx2_fdm) 
par(oldpar)
...
```{r plot_Kx2_res_interaction, fig.cap = 'Residuals and interactions', fig.show = 'hold', out.width='50%'}
#residuals
oldpar <- par(no.readonly = TRUE)
...
plot(fds(Kx2_fdm$residuals$x, Kx2_fdm$residuals$y, xname = "Age"), 
     ...
     plotlegend = TRUE,  legendpos = "bottomleft" #, colorchoice = "gray"
     )
...
Kx_all$rate$male <- Kx_all$rate$male - matrix(Kx2_fdm$basis[, "mean"],
                                              ...
                                              byrow = FALSE) 
plot(Kx_all, main = paste0("interactions (", min(Kx_all$year), " - ",  max(Kx_all$year), ")"),
     ylab = "", plotlegend = TRUE, legendpos = "topleft", # colorchoice = "gray",
...
# Choosing a time series model and forecasting it
...
The estimated values for the $\kappa_j$ before and after outlier removal are shown in Table\ \ref{tab:forecast_rwdrift_coeff}. In both cases we have an obvious drift in the $\kappa_1$, while there is no significant evidence for a drift in $\kappa_2$ and $\kappa_3$.
...
plot_coeff(Kx_forecast, KxTest, ylimList = ylimList)
#
plot_coeff(Kx2_forecast_rw, KxTest, ylimList = ylimList)
```
...
One disadvantage of this model type is that it is difficult to find the parameters $\alpha$ and $\beta$. Often several sets of parameters provide a good fit. In particular, often ones with a large value of $\beta$ fit well, but this means the growth/slope changes very fast, which is usually undesirable. One deals with this problem by restricting $\beta$ to small values. We use Holt's linear model only for the functional data model after outlier removal. The unrestricted parameters are given in the left hand side of Table\ \ref{tab:forecast_ets_coeff} and the time series are plotted in top row of Figure\ \ref{fig:forecast_ets_coeff_plot}. We do in fact see large $\beta_1$ and $\beta_2$, and the high jumps of the second coefficient at the end are extrapolated into the future. While this may look plausible in the figure, remember that it means in reality decreasing health cost at lower ages (even after taking the other components into account) which cannot be true in the long run. If we use the restriction $\beta \le 0.2$, the models for $\kappa_1$ and $\kappa_2$ become random walks with drift. So, to get something new we require $\beta \le 0.25$. Here, $\kappa_1$ has an interesting new model, $\kappa_2$ follows a random walk, and $\kappa_3$ is modeled by a linear regression, see right hand side of  Table\ \ref{tab:forecast_ets_coeff} and bottom row of Figure\ \ref{fig:forecast_ets_coeff_plot}. 
...
plot_coeff(Kx2_forecast_ets_unres, KxTest, ylimList = ylimList)
#, out.width='50%'
plot_coeff(Kx2_forecast_ets, KxTest, ylimList = ylimList)
```




# Forcasting the whole model

In the preceding sections we defined all the components for three stochastic models  --- our favorite model is the one after outlier removal with random walk with drift as time series model, as an alternative we use Holt's linear model as the time series model; finally, we have for comparison the model based on the whole data set with random walk with drift. We are now ready to produce point--forecasts as well as prediction intervals. 

Figure\ \ref{fig:forecast_interaction} shows the age--time--interaction term for the years 2009 to 2011. To get the total claims one has to add the purely age--dependent average level $\mu(x)$. It is left out in order to show the time--dependence more clearly. Adding it one gets graphs like the left hand side of Figure\ \ref{fig:plot_claims_exposure}, which is dominated by the steepness of $\mu(x)$.

The black lines are the actual claims. The red lines are the predicted claims. The gray lines show the best approximation possible to the actual data with the chosen basis functions. The thin red lines are the 90% prediction intervals based on the variance formula of Section\ \ref{functional-data-modelling-approach} and normality assumption. Finally, the yellow shaded areas are the prediction intervals based on Monte Carlo simulation, where the unknown error distributions are simulated by the bootstrap method of @HS09. The latter is implemented in the *simulate* function of the **R** package *demography*.


```{r forecast_interaction, fig.height = 2.9, fig.show = 'hold',  out.width = '100%', fig.cap = 'Forecasts for the years 2009 to 2011'}
# random walk #####
#
...
# ets #####
#
plot_sim_basis(Kx2_forecast_ets, KxTest, KxTest$year, main = "Holt's" , 
               ylim = c(-100, 500), quant.lower = .05, quant.upper = .95)  
